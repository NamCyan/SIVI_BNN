...
class Appr(object):
    ...
    def __init__(self,model,optim = 'Adam', nosample=10,nepochs=100,sbatch=256,lr=0.001,lr_min=1e-6,lr_factor=3,lr_patience=5,clipgrad=100, experiment= None,args=None, log_name=None, split=False):
        self.model=model
    ...
    def eval(self,x,y):
        ...
        with torch.no_grad():
            for i in range(0, len(r), self.sbatch):
                ...
                loss = self.model.loss_forward(images, targets, N_M, self.nosample)
                output = self.model.pred_sample(images, targets, self.nosample)
                _, pred = output.max(1)
