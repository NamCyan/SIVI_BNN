...
for l in lr:
  for t in tau:
    ...
    for split_id in range(n_splits):
        ...
        Appr = appr(model, optim= optim, z_sample= z_sample, nosample= nosample, test_sample= test_sample,nepochs= nepochs, sbatch= sbatch, lr=l, lr_min=lr_min, lr_factor=lr_factor, lr_patience=lr_patience)
        if split_id == 0:
            # model info
