...
def get(data_type, split_id, output_normalize=True):
    ...
    split_train_valid = int(0.8* len(train_input))

    data = {}
    data['train'] = {}
    data['valid'] = {}
    data['test'] = {}
    data['train']['x'] = train_input[0:split_train_valid,:]
    data['train']['y'] = train_output[0:split_train_valid]
    data['valid']['x'] = train_input[split_train_valid:,:]
    data['valid']['y'] = train_output[split_train_valid:]
    data['test']['x'] = test_input
    data['test']['y'] = test_output
    
    #print(len(data['train']['x']), len(data['valid']['x']), len(data['test']['x']))
    ...
    return data, size, normalization_info



if __name__ == '__main__':
    data, size, normalization_info = get('BOSTON', split_id=1, output_normalize=True)
