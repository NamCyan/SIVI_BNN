...
class Net(nn.Module):
    def __init__(self, inputsize, output_dim, SIVI_input_dim, SIVI_layer_size, prior_gmm= True, local_rep= False, droprate= None, ratio= 0.25):
        ...
        self.bfc1= SIVI_bayes_layer(d3,output_dim, SIVI_input_dim= SIVI_input_dim, SIVI_layer_size = SIVI_layer_size, prior_gmm= prior_gmm, droprate= droprate, ratio = ratio)

        self.MaxPool = torch.nn.MaxPool2d(2)  
        self.lrelu = torch.nn.LeakyReLU()

    def forward(self, x, sample=True, train= True):
        h= self.lrelu(self.bn1(self.conv1(x, sample=sample, train= train)))
        h= self.MaxPool(h)
        h= self.lrelu(self.bn2(self.conv2(h, sample=sample, train= train)))
        h= self.lrelu(self.bn3(self.conv3(h, sample=sample, train= train)))
